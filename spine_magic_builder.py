...
def export_set(
    ...
    for idx, pg in enumerate(pages):
        ...
        if chosen and not allow_reuse_textures:
            used.add(chosen)
    ...
    staged = 0
    if stage_dim_candidates and missed_pages:
        for page_idx, page_name, want_w, want_h, prefer_same in missed_pages:
    ...
    if stage_dim_candidates and missed_pages:
        print(f"    staged candidates: {staged} links/files across {len(missed_pages)} unresolved page(s)")
...
def main():
    ...
    ap.add_argument("--root", required=False, help="Root folder to scan (recursively).")
